...
export async function openclawToolsInvoke(args: {
  ...
  const { gatewayUrl, gatewayToken, tool, action, toolArgs, sessionKey, headers } = args;
  ...
  if (!resp.ok) {
    throw new Error(`openclaw_invoke_failed status=${resp.status} body=${text.slice(0, 500)}`);
  }
...
export async function openclawSpawnSession(args: OpenClawSpawnSessionArgs): Promise<unknown> {
  try {
